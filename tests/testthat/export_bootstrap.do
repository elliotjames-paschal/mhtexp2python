program mhtexp2
    ...
    // Export the results matrix to CSV for Python comparison
    mata st_matrix("results", results)
    mata: st_matrix("results_export", results)
    mata: export_results_to_csv("results_export")
end
...
void function export_results_to_csv(string scalar matrix_name){
    result_matrix = st_matrix(matrix_name)
    fh_results = fopen("stata_results.csv", "w")
    fput(fh_results, "outcome,subgroup,t1,t2,coefficient,Remark3_2,Thm3_1,Remark3_8,Bonf,Holm")
    ...
    for (i = 1; i <= rows(result_matrix); i++) {
        ...
        fput(fh_results, row_str)
    }
    fclose(fh_results)
    printf("Stata results exported to stata_results.csv (%g rows)\n", rows(result_matrix))
}
